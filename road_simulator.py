...
""")
    ...
    subtitle=label('01   /   DISTANCE STUDY',(-.75,.42),.62,grey(.4))
    rule(-.63,.401,.24)
    ...
    scale_note=label('MOCK SCALE  1 cm : 0.5 m',(.68,-.347),.48,grey(.4),origin=(0,0))
    ...
    def button(text,x,width,action=None,solid=False):
        ...
        btn=Button(text='',position=(x,-.412,-.01),scale=(width,.046),color=ink if solid else paper,
                   highlight_color=grey(.18) if solid else grey(.88),pressed_color=grey(.23) if solid else grey(.80),
                   on_click=action or (lambda:None))
        # A UI sibling prevents Button's background tint multiplying its text.
        btn.caption=label(text,(x,-.412),.83,paper if solid else ink,origin=(0,0),z=-.045)
        controls.append(btn)
        return btn
    ...
    def update_frame():
        ...
        popup.enabled=bool(sim.night and not sim.done and alpha>0)
